...
def test_indicator_map_coverage_keys():
    assert INDICATOR_MAP["WHS8_110"] == "mcv1_coverage"
...
def test_fetch_returns_most_recent_non_null_year():
    ...
    query = _make_query(indicator_codes=["WHS8_110"])
    with patch("epichat.adapters.who_gho._fetch_text",
               side_effect=_mock_fetch({"WHS8_110": _COVERAGE_RESPONSE})):
...
def test_fetch_multiple_indicators():
    adapter = _make_adapter()
    query = _make_query(indicator_codes=["WHS8_110", "WHS3_62"])
    with patch("epichat.adapters.who_gho._fetch_text",
               side_effect=_mock_fetch({"WHS8_110": _COVERAGE_RESPONSE, "WHS3_62": _CASES_RESPONSE})):
        results = adapter.fetch(query)
